...
class SeparableConvBlock(keras.layers.Layer):
    def __init__(self, num_channels, kernel_size, strides, momentum, epsilon, **kwargs):
        super(SeparableConvBlock, self).__init__(**kwargs)

        self.separable_conv2d = layers.SeparableConv2D(num_channels, kernel_size=kernel_size, strides=strides, padding='same',
                                use_bias=True, name=f'{self.name}/conv')
        self.bn = layers.BatchNormalization(momentum=momentum, epsilon=epsilon, name=f'{self.name}/bn')

    def call(self, inputs):
        out = self.separable_conv2d(inputs)
        out = self.bn(out)
...
class ConvBlock(keras.layers.Layer):
    def __init__(self, num_channels, kernel_size, strides, momentum, epsilon, **kwargs):
        ...
        self.conv2d = layers.Conv2D(num_channels, kernel_size=kernel_size, strides=strides, padding='same',
                       use_bias=True, name='{}/conv'.format(self.name))
        self.bn = layers.BatchNormalization(momentum=momentum, epsilon=epsilon, name='{}/bn'.format(self.name))
        self.relu = layers.ReLU(name='{}/relu'.format(self.name))
...
def bbox_transform_inv(anchors, regression):
