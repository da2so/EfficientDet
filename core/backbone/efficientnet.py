...
def EfficientNet(
    ...
    # find output features
    output_features = []
    tmp_f = None 
    for layer in model.layers:
        l_name = layer.name
        if tmp_f != None:
            if tmp_f not in l_name :
                output_features += [prev_layer.output]
                features.remove(tmp_f)
                tmp_f = None
    ...
    # reconstruct model for object detection
    model = training.Model(model.input, output_features, name=model_name)
    return model
